...
void SourceWidget::setAlphaImage(const QImage &img){
	if(image.isNull())
	{
		QImage image(":/images/alpha.jpg");
		alpha=image;
	}
	else
		alpha = img;
...
//			return;
		...
		//**********************************************
		//sourcePatch.save("aaaaaaaa.png");
        
        // Pass source patch pixels to glWidet
        glWidget->setSourcePatch(sourcePatch);

		QImage alphaPatch = alpha.copy(boundingRect.toRect().adjusted(-1,-1,1,1)); //***********  addition ***********
		glWidget->setAlphaPatch(alphaPatch);
