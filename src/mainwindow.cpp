...
void MainWindow::quickOpen(){
	...
	QString alpha(":/images/source.jpg");
#ifdef MACOSX
	setResourcesDir();
#endif
    setImages(source, target , alpha);
...
void MainWindow::openImages(){
	...
	QString alpha = QFileDialog::getOpenFileName(this,
        tr("Open alpha Image"), QDir::currentPath(), tr("Image Files (*.png *.jpg *.bmp)"));//******************* addition ***************

    setImages(source, target , alpha);//************** modified ***********

}

void MainWindow::setImages(QString & source, QString & target ,QString & alpha){
	...
	if (!alpha.isEmpty()) {
        ...
        if (image.isNull()) {
            QMessageBox::information(this, tr("MVCloner"),
                                     tr("Cannot load %1.").arg(alpha));
            return;
        }
        sourceWindow->setAlphaImage(image);
        sourceWindow->setFixedSize(image.width(), image.height());
        //sourceWindow->show();
     }
